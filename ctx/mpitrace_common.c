              ...
              int src, int dest, int bytes, MPI_Comm comm)
{
   ...
   {
      bin = 0; 
      bin_count[ctx][id][bin] ++;
      bin_bytes[ctx][id][bin] += (double) bytes;
      bin_time[ctx][id][bin] += timediff;
   }
   ...
   event_count[ctx][id] ++;
   total_time[ctx][id ] += timediff;
   if (bytes >= 0) total_bytes[ctx][id] += (double) bytes;

   // explicitly force the default context to include all calls from other contexts
   if (ctx != 0)
   {   
      // note: bin is already set if it is used here
      if (bytes >= 0)
      {   
         bin_count[0][id][bin] ++; 
         bin_bytes[0][id][bin] += (double) bytes;
         bin_time[0][id][bin] += timediff;
         total_bytes[0][id] += (double) bytes;
      }   

      event_count[0][id] ++; 
      total_time[0][id] += timediff;
   }
   ...
   if ( comm_profile && (comm != MPI_COMM_NULL) )
   {
      ...
      comm_count[ctx][id][bin] ++;
      comm_bytes[ctx][id][bin] += (double) bytes;
      comm_time[ctx][id][bin]  += timediff;
      total_size[ctx][bin]     += (double) size;

      if (ctx != 0)
      {   
         comm_count[0][id][bin] ++; 
         comm_bytes[0][id][bin] += (double) bytes;
         comm_time[0][id][bin]  += timediff;
         total_size[0][bin]     += (double) size;
      }
   }
...
void initialize_summary_data(void)
{
    ...
    {   
       ...
       context_tbeg[context] = 0.0;
       context_elapsed[context] = 0.0;
    ...
    {   
       for (context=0; context<MAX_CONTEXTS; context++)
       {
          for (id=0; id<MAX_IDS; id++)
          {
             for (bin=0; bin<MAX_BINS; bin++)
             {
                comm_count[context][id][bin] = 0LL;
                comm_bytes[context][id][bin] = 0.0;
                comm_time[context][id][bin]  = 0.0;
             }
          }

          for (bin=0; bin<MAX_BINS; bin++) total_size[context][bin] = 0;
       }
    }
    ...
    if (collective_barrier) for (context=0; context<MAX_CONTEXTS; context++) synctime[context] = 0.0;

    maxctx = 0;
    mpitrace_Context_start("default");
...
/*==============================================*/
/* Start collecting data for a named code block */
/*==============================================*/
void mpitrace_Context_start(const char * this_label)
{
   int i, match;
   char * ptr;
   double tbeg;
   ...
   gettimeofday(&TV, NULL);
   tbeg = (double) TV.tv_sec + 1.0e-6 * ((double) TV.tv_usec);

   if (maxctx < MAX_CONTEXTS)
   {   
       match = 0;
       for (i=maxctx-1; i>=0; i--)
       {
           if (0 == strcmp(context_label[i], this_label))
           {
               match = 1;
               break;
           }
       ...
       {
           i = maxctx;
           ptr = strcpy(context_label[i], this_label);
           if (ptr == NULL) context_label[i][0] = '\0';
           maxctx ++; 
       }
...
/* version for Fortran                          */
/*==============================================*/
#pragma weak mpitrace_context_start_=mpitrace_context_start
void mpitrace_context_start(const char * f_label, int length)
{
   int i, match;
   char * ptr;
   double tbeg;
...
/*=============================================*/
/* Stop collecting data for a named code block */
/*=============================================*/
void mpitrace_Context_stop(const char * this_label)
{
   int i, match;
   double tend;
   struct timeval TV;
   ...
   gettimeofday(&TV, NULL);
   tend = (double) TV.tv_sec + 1.0e-6 * ((double) TV.tv_usec);
   ...
   if (maxctx < MAX_CONTEXTS)
   {
       ...
       if (match == 1)
       {
          context_elapsed[i] += (tend - context_tbeg[i]);
          context_stops[i] ++;
       }
   }

   ctx = 0;   // set the global context variable to zero after each block
...
/* Fortran version.                            */
/*=============================================*/
#pragma weak mpitrace_context_stop_=mpitrace_context_stop
void mpitrace_context_stop(const char * f_label, int length)
{
   int i, match;
   double tend;
   struct timeval TV;
...
{
   int i, k, rc, id, bin, myrank, context, numcontexts, print_summary;
   ...
   double context_comm[MAX_CONTEXTS];
   FILE * fh, * fp;
...
//    context_switches = RU.ru_nvcsw + RU.ru_nivcsw - context_switches_initial;
   ...
   /* the default context zero is inclusive by construct.*/
   /*----------------------------------------------------*/
   total_comm = 0.0;
   for (id=0; id<MIN_MPI_IO_ID; id++)
   {
     if (event_count[0][id] > 0LL)
     {
       total_comm += total_time[0][id];
     }
   }

   context_comm[0] = total_comm;

   for (context=1; context<numcontexts; context++)
   {   
      context_comm[context] = 0.0;
      for (id=0; id<MIN_MPI_IO_ID; id++)
      {   
        if (event_count[context][id] > 0LL)
        {
          context_comm[context] += total_time[context][id];
        }
      }   

      if (collective_barrier)
      {   
          context_comm[context] += synctime[context];
          synctime[0]           += synctime[context];
      }   
   }   
   ...
   for (context=0; context<numcontexts; context++)
   {
      for (id=MIN_MPI_IO_ID; id<MAX_IDS; id++)
      {
        if (event_count[context][id] > 0LL)
        {
          mpi_io_time += total_time[context][id];
   ...
   if (output_filter)
   {
      ...
      // begin loop over contexts
      for (context=0; context<numcontexts; context++)
      {
         if (context_starts[context] == context_stops[context] )
         {
            if (context == 0)
            {
               if (comm_profile)
            ...
            else fprintf(fh,"\nData for context = %s, call count = %ld\n", context_label[context], context_starts[context]);
            fprintf(fh,"-----------------------------------------------------------------------\n");
            ...
            for (id=0; id<MAX_IDS; id++)
            {
              if (event_count[context][id] > 0LL)
              {
                avg_bytes = total_bytes[context][id] / ((double) event_count[context][id]);
                fprintf(fh,sformat, label[id], event_count[context][id], avg_bytes, total_time[context][id]);
              }
            }
            ...
            if (context == 0)
            {
               ...
               fprintf(fh,"max resident set size    = %.3f MBytes.\n", max_memory);
            ...
            for (id=0; id<MAX_IDS; id++)
            {
              if ( event_count[context][id] > 0LL  &&  total_bytes[context][id] > 0.0 )
              {
                fprintf(fh,"%-22s    #calls    avg. bytes      time(sec)\n", label[id]);
                for (bin=0; bin<MAX_BINS; bin++)
                {
                  if (bin_count[context][id][bin] > 0LL)
                  {
                    avg_bytes = bin_bytes[context][id][bin] / ((double) bin_count[context][id][bin] );
                    fprintf(fh,dformat, bin_count[context][id][bin], avg_bytes, bin_time[context][id][bin]);
                  }
                }
                fprintf(fh,"\n");
              }
            ...
            if (comm_profile)
            {
               ...
               for (bin=0; bin<MAX_BINS; bin++)
               {
                  total_count = 0.0;
                  total_comm = 0.0;
                  for (id=0; id<MIN_MPI_IO_ID; id++) total_count += (double)  comm_count[context][id][bin];
                  if (total_size[context][bin] > 0.0)
                  {
                     if (bin == 0) min_size = 0;
                     else          min_size = comm_size[context][bin-1] + 1;
                     max_size = comm_size[context][bin];
                     avg_size = total_size[context][bin]/total_count; 
                     fprintf(fh,"\n");
                     fprintf(fh,"   -----------------------------------------------------------------------\n");
                     ...
                     for (id=0; id<MAX_IDS; id++)
                     {
                       if ( comm_count[context][id][bin] > 0LL )
                       {
                          total_comm += comm_time[context][id][bin];
                          if (comm_bytes[context][id][bin] < 0.0) avg_bytes = 0.0;
                          else avg_bytes = comm_bytes[context][id][bin] / ((double) comm_count[context][id][bin] );
                          fprintf(fh, pformat, label[id], comm_count[context][id][bin], avg_bytes, comm_time[context][id][bin]);
                       }
                     }
...
static void write_profile_data_myrank(void)
{
   ...
   int tag = 100;
   char * ptr;
   char filename[240], pattern_file[240];
   struct maxStruct {
                       float data;
                       int rank;
                    };
      ...
      for (context=0; context<numcontexts; context++)
      {
         if (context_starts[context] == context_stops[context] )
         {
            ...
            if (context == 0)
            {
               ...
               fprintf(fh,"\n");
               fprintf(fh,"Rank %d reported the largest memory utilization : %.2f MBytes\n", max_mem.rank, max_mem.data);
               fprintf(fh,"Rank %d reported the largest elapsed time : %.2f sec\n", max_elapsed.rank, max_elapsed.data);
               fprintf(fh,"\n");
            }
            fprintf(fh,"-----------------------------------------------------------------------\n");
